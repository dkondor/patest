...
pid_t popen_noshell_vmfork(int (*fn)(void *), void *arg, void **memory_to_free_on_child_exit) {
		...
		*memory_to_free_on_child_exit = stack;
		...
		/* note: CLONE_UNTRACED necessary so that gdb will keep working in the parent process
		 * see: https://github.com/famzah/popen-noshell/issues/10 */
		pid = clone(fn, stack_aligned, CLONE_VM | CLONE_UNTRACED | SIGCHLD, arg);
#else
