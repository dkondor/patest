...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
  ...
  newTree->npar = npar;
  newTree->childrentmp = (double*)SafeMalloc(sizeof(double)*2*npar);
  newTree->childrentmp2 = newTree->childrentmp + npar;
  ...
  temp->red=0;
  temp->key=0;
  temp->children = (double*)SafeMalloc(sizeof(double)*npar);
  for(i=0;i<npar;i++) temp->children[i] = 0.0;
...
static inline void TreeUpdateSum(rb_red_blk_tree* tree, rb_red_blk_node* x) {
	int i;
	for(i=0;i<tree->npar;i++)
		x->children[i] = x->left->children[i] + x->right->children[i] + tree->DistFunc(x->key,tree->dfparam[i]);
}
...
*************************************/
  for(i=0;i<tree->npar;i++) z->children[i] = tree->DistFunc(z->key,tree->dfparam[i]);
  {
       rb_red_blk_node* w = z->parent;
       while(w != root) {
            for(i=0;i<tree->npar;i++) w->children[i] += z->children[i];
            w = w->parent;
       }
  }
  ...
  for(i=0;i<tree->npar) {
    Assert((tree->nil->children[i] == 0.0),"nil->children != 0 in TreeInsertHelp!\n");
    Assert((tree->root->children[i] == 0.0),"root->children != 0 in TreeInsertHelp!\n");
}
#endif
}
...
rb_red_blk_node * RBTreeInsert(rb_red_blk_tree* tree, void* key, void* info) {
  ...
  x->key=key;
  x->info=info;
  x->children = (double*)SafeMalloc(sizeof(double)*tree->npar);
...
void GetNodeRank(rb_red_blk_tree* tree,rb_red_blk_node* x,double* r) {
     rb_red_blk_node* nil = tree->nil;
     rb_red_blk_node* root = tree->root;
     int i;
...
#endif
     for(i=0;i<tree->npar;i++) r[i] = x->left->children[i]; //x is at least this
     rb_red_blk_node* w = x;
     while(w->parent != root) {
          if(w == w->parent->right) for(i=0;i<tree->npar;i++) r[i] += w->parent->left->children[i] + tree->DistFunc(w->parent->key,tree->dfparam[i]);
          w = w->parent;
     }
}
...
void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != tree->nil) {
    ...
    GetNodeRank(tree,x,tree->childrentmp);
    printf("ranks:");
    for(i=0;i<tree->npar;i++) printf(" %g",tree->childrentmp[i]); /** print the rank of node also **/
    printf("  info=");
...
void RBTreeDestroy(rb_red_blk_tree* tree) {
  ...
  free(tree->nil->children);
  free(tree->root);
  free(tree->nil);
  free(tree->childrentmp);
  free(tree);
}
...
void RBDelete(rb_red_blk_tree* tree, rb_red_blk_node* z){
  ...
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node* root=tree->root;
  double* ydval = tree->childrentmp; /* DistFunc(y) */
  double* zdval = tree->childrentmp2; /* DistFunc(x) */
  int i;
  ...
  for(i=0;i<tree->npar;i++) ydval[i] = tree->DistFunc(y->key,tree->dfparam[i]);
  {
       rb_red_blk_node* w = y->parent;
       
       while(w != root) {
            for(i=0;i<tree->npar;i++) w->children[i] -= ydval[i];
            w = w->parent;
       }
  }
 ...
 /*  if(x != nil) x->children = y->children; //!! we put x in the place of y */
  ...
  if (y != z) { /* y should not be nil in this case */
      ...
      ********************************************/
      for(i=0;i<tree->npar;i++) zdval[i] = tree->DistFunc(z->key,tree->dfparam[i]);
#ifdef DEBUG_ASSERT
    ...
    y->parent=z->parent;
    y->red=z->red;
    for(i=0;i<tree->npar;i++) y->children[i] = y->left->children[i] + y->right->children[i] + ydval[i];
    ...
    {
         rb_red_blk_node* w = y->parent;
         for(i=0;i<tree->npar;i++) ydval[i] -= zdval[i];
